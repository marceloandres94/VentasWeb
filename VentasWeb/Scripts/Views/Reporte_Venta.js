...
$('#btnBuscar').on('click', function () {
    ...
    jQuery.ajax({
        url: $.MisUrls.url._ObtenerIndicadorVenta + "?fechainicio=" + $("#txtFechaInicio").val() + "&fechafin=" + $("#txtFechaFin").val() + "&idtienda=" + $("#cboTienda").val(),
        type: "GET",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            ...
            if (data != undefined && data != null) {
                ...
                $("#txtIndicadorVenta").html("");


                $.each(data, function (i, row) {

                    $("#txtIndicadorVenta").append(

                        $("#txtIndicadorVenta").val(row["VentasRealizadas"]),

                    ).appendTo("#txtIndicadorVenta");
                })
