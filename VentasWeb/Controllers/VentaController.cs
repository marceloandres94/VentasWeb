...
namespace VentasWeb.Controllers
{
    public class VentaController : Controller
    {
            ...
            if (oVenta == null)
                ...
                oVenta.oListaDetalleVenta = (from dv in oVenta.oListaDetalleVenta
                                             select new DetalleVenta()
                                             {
                                                 ...
                                                 TextoPrecioUnidad = dv.PrecioUnidad.ToString(),
                                                 ImporteTotal = dv.ImporteTotal,
                                                 TextoImporteTotal = dv.ImporteTotal.ToString()
                                             }).ToList();

                oVenta.TextoImporteRecibido = oVenta.ImporteRecibido.ToString();
                oVenta.TextoImporteCambio = oVenta.ImporteCambio.ToString();
                oVenta.TextoTotalCosto = oVenta.TotalCosto.ToString();
            }
        ...
        public JsonResult ObtenerProductoParaVenta(int IdTienda)
        {

            List<ProductoTienda> oListaProductoTienda = CD_ProductoTienda.Instancia.ObtenerProductoParaVenta();
            oListaProductoTienda = oListaProductoTienda.Where(x => x.oTienda.IdTienda == IdTienda && x.Stock > 0).ToList();
        ...
        [HttpPost]
        public JsonResult ObtenerUltimasVenta()
        {
            List<Venta> lista = CD_Venta.Instancia.ObtenerUltimasVenta();
            return Json(new { data = lista }, JsonRequestBehavior.AllowGet);
        }
    }
}